...
def extract_text_from_file(uploaded_file):
    """Автоматически извлекает текст из PDF или DOCX файлов"""
    try:
...
def calc_score(resume_text, role, experience):
    ...
    if not categories:
        return {"total": 0, "details": {}, "summary": "Не удалось определить требования к вакансии."}
    ...
    # ИИ-Генерация саммари на основе контекста (Симуляция LLM-анализа)
    if total_score >= 70:
...
def show_candidate_modal(row, res_details):
    ...
    # Выводим Умное ИИ-Саммари из базы (или пересчитываем, если старая запись)
...
# 6. ОСНОВНОЙ БИЗНЕС-ИНТЕРФЕЙС
    ...
    if st.session_state.user_role in ['admin', 'recruiter']:
        ...
        # Новый виджет умной загрузки файлов вне формы для мгновенного чтения текста
        ...
        with st.form("resume_form"):
            ...
            with col2:
                ...
                # Если файл загружен, автоматически подставляем текст в поле
            ...
            if st.form_submit_button("🔥 Запустить ИИ-скрининг и добавить в воронку", use_container_width=True):
                if name and text:
                    # Рассчитываем ИИ-метрики и генерируем саммари перед сохранением
                    ...
                    st.error("Пожалуйста, заполните ФИО кандидата и убедитесь, что текст резюме извлечен или вставлен.")
        st.write("---")
    ...
    if st.session_state.user_role in ['admin', 'manager']:
        ...
        conn = sqlite3.connect('talent_hub.db')
        df = pd.read_sql("SELECT * omnibus FROM resumes", conn)
        # Обработка на случай пустой старой БД
...
def show_candidate_modal(row, res_details):
    ...
    st.markdown("**Выдержка из резюме:**")
    st.info(row['content'])
    
    st.markdown("**Метрики соответствия ИИ-матрице (Hard-Skills):**")
    for skill, val in res_details['details'].items():
        st.write(f"- {skill}: {val}%")
        st.progress(val / 100)
    st.write("---")
...
# 6. ОСНОВНОЙ БИЗНЕС-ИНТЕРФЕЙС
    ...
    if st.session_state.user_role in ['admin', 'recruiter']:
        ...
        with st.form("resume_form"):
            ...
            with col2:
                years = st.number_input("Подтвержденный стаж (лет)", min_value=0, max_value=40, value=0)
                text = st.text_area("Ключевые навыки / Текст резюме", placeholder="Перечислите навыки...")
