...
# Защита от спама запросами (Rate Limiting)
...
# --- РЕЖИМ 2: ОЦЕНКА НЕДВИЖИМОСТИ ---
    ...
    st.metric(label="Прогнозная цена объекта", value=f"${max(final_price, 15000.0):,.2f}")    st.metric(label="Прогнозная цена объекта", value=f"${max(final_price, 15000.0):,.2f}")    if prob >= 0.5: st.success("🤖 Робот: ОДОБРЕНО")
