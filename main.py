...
def ask_llm_analysis(resume_text, role, experience, requirements):
    if not OPENROUTER_API_KEY: return "⚠️ Ошибка: API-ключ не настроен."
    prompt = f"Ты HR-директор Blackwood. Проанализируй: {resume_text}. Вакансия: {role}. Дай отчет в Markdown."
    try:
