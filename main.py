...
def show_candidate_modal(row, res_details):
    ...
    # Вернули кастомный блок-карточку внутрь поп-апа
    st.markdown(f"""
    <div class="modal-card">
        <h4>Выдержка из резюме:</h4>
        <p style="font-style: italic; color: #444444;">"{row['content']}"</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        with tab_crm:
            ...
            for _, row in filtered_df.sort_values(by='Score', ascending=False).iterrows():
                ...
                # Построение красивой строки кандидата (Аватар + Данные + Кнопки)
                with st.container():
                    col_img, col_main, col_btns = st.columns([1, 4, 2])
                    ...
                    with col_btns:
                        ...
                        with col_sub1:
                            if st.button("💾 Сохранить", key=f"upd_{row['id']}", use_container_width=True):
                                ...
                                st.rerun()
                        with col_sub2:
                            # Полностью восстановили функцию удаления
                            if st.button("🗑️ Удалить", key=f"del_{row['id']}", use_container_width=True):
                                conn = sqlite3.connect('talent_hub.db')
                                conn.execute("DELETE FROM resumes WHERE id=?", (row['id'],))
                                conn.commit()
        ...
        # ВКЛАДКА 2: Финансовые и конверсионные метрики (ПОЛНАЯ ВЕРСИЯ)
        with tab_metrics:
            st.write("")
            col_m1, col_m2, col_m3 = st.columns(3)
        ...
        # ВКЛАДКА 3: Генератор Job Offers
        with tab_offers:
            st.write("")
            offer_candidates = df[df['normalized_status'] == "Оффер"]
